...
let isAuthorized = (req, res, next) => {
    console.log("In is authorized method");
  if (req.params.authToken || req.query.authToken || req.body.authToken || req.header('authToken')) {
    Auth.findOne({authToken: req.header('authToken') || req.params.authToken || req.body.authToken || req.query.authToken}, (err, authDetails) => {
      ...
      } else if (check.isEmpty(authDetails)) 
      {
        ...
        logger.error('No AuthorizationKey ', 'AuthorizationMiddleware', 10)
        let apiResponse = responseLib.generate(true, 'Invalid Or Expired AuthorizationKey', 404, null)
        res.send(apiResponse)
      } else {
        token.verifyToken(authDetails.authToken,"someveryrandomstringthatnobodycanguess",(err,decoded)=>{
            if(err){
              console.log("this is secret "+authDetails.tokenSecret)
